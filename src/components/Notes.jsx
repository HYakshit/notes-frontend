...
const Notes = ({ notes, loading, setNotes }) => {
  ...
  if (!notes || notes.length === 0) return <div className="flex justify-center items-center h-screen">No notes available</div>;
        ...
        {notes.map((note) => (
          <div key={note.id} className="bg-white p-4 rounded-lg shadow-md w-full">
            ...
            <div className="flex justify-end gap-2 mb-1">
              <button onClick={() => handleEditNote(note)} className="btn btn-xs bg-yellow-300">Edit</button>
              <button onClick={() => deleteNote(note.id)} className="btn btn-xs bg-red-400">Delete</button>
              <button onClick={() => pinNote(note.id)} className="btn btn-xs bg-blue-400">{note.pinned ? "Unpin" : "Pin"}</button>
            </div>

            {/* Collapse card stays exactly as-is */}
            <div
